...
import {KubernetesCluster, Vpc, getKubernetesVersions} from "@pulumi/digitalocean";
...
export class Cluster extends ComponentResource {
  ...
  constructor(name: string, args: Args, opts?: ResourceOptions) {
    ...
    this.vpc = new Vpc(`${name}-vpc`, {region: args.region}, defaultResourceOptions);

    // Create the cluster with the default version
    const latestVersion = getKubernetesVersions({}, defaultResourceOptions).then(v => v.latestVersion);
    this.k8s = new KubernetesCluster(`${name}-k8s`, {
